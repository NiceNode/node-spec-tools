 ...
 * @returns 
 */
export const calcNewControllerConfig = (newSpec: NodeSpecification, currentControllerConfig: ConfigValuesMap): ConfigValuesMap => {
    ...
    for(const [key, value] of Object.entries(currentControllerConfig)) {
        ...
        if(newTranslationKeys.includes(key)) {
            if(newTranslations[key]?.uiControl?.type.includes('select')) {
                ...
                if(newTranslations[key]?.uiControl?.type === "select/multiple") {
                    ...
                    if(Array.isArray(value)) {
                        ...
                        console.error(`Value for multi select should be an array. key: ${key} value: ${value}`)
                    }
                    ...
                    if(typeof value === 'string') {
                        if(newSelectOptions.map(option => option.value).includes(value)) {
                            newControllerConfig[key] = value;
                        }
                    } else {
                        console.error(`Value for single select should be a string. key: ${key} value: ${value}`)
                    }
                }
            } else {
            ...
            console.log(`Current Controller Config key NOT found :: ${key}: ${value}`);
        }
    }
