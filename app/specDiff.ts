...
export const calcUserSpecDiff = (oldSpec: NodeSpecification, newSpec: NodeSpecification): UserSpecDiff[] => {
    ...
    if(!compareObjects(oldSysReq, newSysReq)) {
        ...
        diffs.push({ message: `System requirements: ${oldSysReqString} -> ${newSysReqString}`})
    }
    ...
    for(const key of oldTranslationKeys) {
        if(!newTranslationKeys.includes(key)) {
            diffs.push({ message: `Removed setting: ${oldTranslations[key]?.displayName}`})
        } else {
            if (oldTranslations[key]?.uiControl?.type.includes('select')) {
                const oldSelectOptions = (oldTranslations[key].uiControl as SelectControl)?.controlTranslations as SelectTranslation[];
                const newSelectOptions = (newTranslations[key].uiControl as SelectControl)?.controlTranslations as SelectTranslation[];
                if (!compareObjects(oldSelectOptions, newSelectOptions)) {
                    diffs.push({ message: `Changed setting options: ${JSON.stringify(oldSelectOptions)} -> ${JSON.stringify(newSelectOptions)}` });
                }
            }
    ...
    for(const key of newTranslationKeys) {
        if(!oldTranslationKeys.includes(key)) {
            diffs.push({ message: `New setting: ${newTranslations[key]?.displayName}`})
        }
